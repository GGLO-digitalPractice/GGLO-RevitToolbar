...
def test_selected_family(chosen_color):
    """Prompt user to select a single family and test it."""
    
    # Prompt user to select a family
    all_families = FilteredElementCollector(doc).OfClass(Family).ToElements()
    ...
    # If a family is selected, run the test
    if selected_name:
        family = next((fam for fam in all_families if fam.Name == selected_name), None)
        if family:
            print("Testing...")
            process_family_from_object(doc, family, lambda fam_doc: modify_family_font_and_color(fam_doc, chosen_font, chosen_color))
        else:
